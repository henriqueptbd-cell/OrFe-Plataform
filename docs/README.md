# 📚 Documentação OrFe

**Status do Projeto:** 🚧 MVP em Desenvolvimento  
**Cliente Piloto:** Igreja da esposa  
**Objetivo:** Validar com usuários reais antes de escalar

---

## 📂 Índice

### 📦 Produto

- [Visão do Produto e MVP](./produto/visao-e-mvp.md) - O que é, escopo, critérios de sucesso
- [Roadmap](./produto/roadmap.md) - Visão de longo prazo e próximas versões

### 🏃 Scrum

- [Product Backlog](./scrum/product-backlog.md) - Todas as histórias priorizadas
- [Sprint 0](./scrum/sprint-0.md) - Setup & Fundação
- [Sprint 1](./scrum/sprint-1.md) - Painel Administrativo
- [Sprint 2](./scrum/sprint-2.md) - Área Pública e Interação
- [Sprint 3](./scrum/sprint-3.md) - Gestão e Feed Social
- [Sprint 4](./scrum/sprint-4.md) - Polimento e Validação

### ⚙️ Técnico

- [Stack e Arquitetura](./tecnico/stack-e-arquitetura.md) - Tecnologias, estrutura de pastas, padrões
- [Banco de Dados](./tecnico/banco-de-dados.md) - Modelagem, coleções, relacionamentos
- [Segurança e Performance](./tecnico/seguranca-e-perf.md) - Auth, RBAC, metas de performance
- [Schemas](./tecnico/schemas.md) - Estrutura das coleções no Firestore
- [Componentes Frontend](./tecnico/componentes-frontend.md) - Estratégia de desenvolvimento, padrões e exemplos

### 🎯 Funcionalidades

- [Autenticação e Permissões](./funcionalidades/autenticacao.md) - Login, papéis, endpoints
- [Perfil do Usuário](./funcionalidades/perfil-usuario.md) - Perfil global, bio, feed pessoal
- [Igreja e Página Pública](./funcionalidades/igreja-e-pagina-publica.md) - Configurações, feed público
- [Posts e Conteúdo](./funcionalidades/posts-e-conteudo.md) - CRUD, comentários, moderação
- [Eventos e Inscrições](./funcionalidades/eventos-e-inscricoes.md) - CRUD, formulários dinâmicos, exportação

---

## 🗺️ Mapa da Documentação

```
docs/
├── README.md ← Você está aqui
│
├── produto/
│ ├── visao-e-mvp.md ← O QUE é o projeto
│ └── roadmap.md ← QUANDO será entregue
│
├── scrum/
│ ├── product-backlog.md ← LISTA completa de histórias
│ ├── sprint-0.md ← COMO começar
│ ├── sprint-1.md
│ ├── sprint-2.md
│ ├── sprint-3.md
│ └── sprint-4.md
│
├── tecnico/
│ ├── stack-e-arquitetura.md ← COM QUE tecnologias
│ ├── banco-de-dados.md ← ONDE os dados ficam
│ └── seguranca-e-perf.md ← COMO proteger
│
└── funcionalidades/
├── autenticacao.md ← Login e permissões
├── perfil-usuario.md ← Perfil global
├── igreja-e-pagina-publica.md
├── posts-e-conteudo.md
└── eventos-e-inscricoes.md

```

---

## 🔗 Links Rápidos

- **Repositório:** [definir]
- **Ambiente de Teste:** [definir]
- **Produção:** [definir]
- **Board de Tarefas:** [definir]
